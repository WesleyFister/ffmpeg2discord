...
class encode(QThread):
    ...
    def calculateDuration(self, duration):
        if self.startTime != "" and self.endTime == "":
            ...
            duration = utils.convertTimeToSeconds(
                self.endTime
            ) - utils.convertTimeToSeconds(self.startTime)
    ...
    def calculateBitrate(
        ...
        if container == "mp4":
            ...
            containerConstOverhead = (
                1024 * 8
            )  # MKV seems to use ~624 bytes however leaning on the safe side
            containerpacketOverhead = (
                12 * 8
            )  # MKV seems to use ~6.25 to 9 bytes per packet however leaning on the safe side
        ...
        if videoOnly == False:
            ...
            if audioOnly == True:
                if self.audioFormat == "WEBM (Video)":
                    ...
                    containerpacketOverhead = (
                        64 * 8
                    )  # MKV seems to use ~52 bytes per audio packet and ~12 per video packet
                    mult = 0.95
                    ...
                    containerpacketOverhead = (
                        52 * 8
                    )  # MKV seems to use ~52 bytes per audio packet
                    mult = 0.95

                elif self.audioFormat == "OGG":
                    containerConstOverhead = (
                        1024 * 8
                    )  # OGG seems to use ~624 bytes however leaning on the safe side
                    containerpacketOverhead = (
                        16 * 8
                    )  # OGG seems to use ~16 bytes per packet
                    mult = 0.95

                audioOverhead = (
                    containerpacketOverhead * fileInfo["numberOfAudioPackets"]
                ) + containerConstOverhead
                audioBitrate = int(
                    ((self.targetFileSize - audioOverhead) / duration) * mult
                )
                upperBound = (
                    512000  # Anything higher than 512kbps is unsupported by Opus
                )
            ...
            # Only encoding audio to be used in a video
                ...
                audioBitrate = int(
                    (((self.targetFileSize - audioOverhead) / 10) / duration) * mult
                )  # 10 is arbitrary. It ensures that 90% of the file size is used for video, and 10% is used for audio
                upperBound = 128000  # 128kbps Opus audio is considered transparent by many people

            if (
                fileInfo["audioBitrate"] <= audioBitrate
                and fileInfo["audioBitrate"] <= upperBound
            ):
                if (
                    fileInfo["audioCodec"] == "opus"
                    and self.normalizezAudio == False
                    and (audioCodec == "opus" or audioCodec == None)
                    and (self.mixAudio == False or fileInfo["audioStreams"] == 1)
                    or fileInfo["audioCodec"] == "aac"
                    and self.normalizezAudio == False
                    and (audioCodec == "aac" or audioCodec == None)
                    and (self.mixAudio == False or fileInfo["audioStreams"] == 1)
                ):
                    audioBitrate = "copy"
        ...
        # Only encoding video
            ...
            videoOverhead = (
                containerpacketOverhead * fileInfo["numberOfVideoPackets"]
                + containerConstOverhead
            )
            videoBitrate = int(
                (
                    (self.targetFileSize - videoOverhead - (audioPath * 8))
                    / fileInfo["videoLength"]
                )
                * mult
            )
    ...
    def encodeVideo(
        ...
        trimFlags, fileInfo["videoLength"] = self.calculateDuration(
            fileInfo["videoLength"]
        )
        ...
        # Otherwise encode audio seperately from video.
            ...
            audioPath = self.encodeAudio(
                filePath, fileInfo, container, audioCodec, audioOnly=False
            )
            if audioPath == "error":
                return "error"
            ...
            ffmpegCommand.extend(
                ["-i", str(audioPath), "-map", "1:a:0", "-c:a", "copy"]
            )
            ffmpegCommand.extend(codec_flags)

        videoBitrate = self.calculateBitrate(
            fileInfo,
            container,
            fileInfo["videoLength"],
            audioCodec,
        ...
        heights = [
            ...
            144,
            72,
        ]  # Divisible by 8 16:9 resolutions
        ...
        currentPixelsPerSecond = (fileInfo["width"] * fileInfo["height"]) * fileInfo[
            "framerate"
        ]
        ...
        if currentPixelsPerSecond <= pixelsPerSecond:
            ...
            if (
                currentPixelsPerSecond >= (pixelsPerSecond * 4)
                and (fileInfo["framerate"] / 2) != 24
            ):  # 4 is arbitrary
            ...
            for testHeight in heights:
                ...
                if (
                    testPixelsPerSecond <= pixelsPerSecond
                    and testHeight < fileInfo["height"]
                ):
                    height = testHeight
                    break
        ...
        ffmpegCommand.extend(
            ["-vf", f"scale='trunc(oh*a/2)*2:{height}':flags=bicubic,format=yuv420p"]
        )
        ...
        def ffmpeg2pass(flags, twoPass, progressPercent, progressPercent3):
            ...
            for progress in ff.run_command_with_progress(
                utils.createNoWindow(), duration_override=fileInfo["videoLength"]
            ):
                ...
                self.updateProgressBar.emit(
                    (progress / progressPercent) + progressPercent3
                )
        ...
        try:
            ...
            ffmpeg2pass(
                ...
                progressPercent3=progressPercent3,
            )
            if self.running == False:
                print("Operation was canceled by user")
                utils.cleanUp(logFile, audioPath, audioExists)
    ...
    def encodeAudio(
        self, file, fileInfo, container=None, audioCodec=None, audioOnly=True, mult=1.0
    ):
        trimFlags, fileInfo["audioDuration"] = self.calculateDuration(
            fileInfo["audioDuration"]
        )
        ...
        if audioOnly == True:
            outputFile = (
                fileInfo["dirName"] + fileInfo["fileName"] + "_FFmpeg2Discord" + "."
            )

            if self.audioFormat == "WEBM (Video)":
                audioCodec = "libopus"
                container = "webm"
                encodeAudioCommand.extend(
                    [
                        "-f",
                        "lavfi",
                        "-i",
                        "color=black:size=160x120",
                        "-map",
                        "1:v:0",
                        "-r",
                        "1",
            ...
            audioBitrate = self.calculateBitrate(
                fileInfo,
                container,
                fileInfo["audioDuration"],
                audioCodec,
            ...
            outputFile = (
                os.getcwd() + "/" + fileInfo["fileName"] + os.urandom(8).hex() + "."
            )
        ...
        if self.normalizezAudio == True and (
            self.mixAudio == True and fileInfo["audioStreams"] > 1
        ):
            encodeAudioCommand.extend(
                [
                    "-filter_complex",
                    f"loudnorm,amerge=inputs={fileInfo['audioStreams']}[a]",
                    "-map",
                    "[a]",
                ]
            )  # Normalizes the first audio track only?
            ...
            encodeAudioCommand.extend(
                [
                    "-filter_complex",
                    f"amerge=inputs={fileInfo['audioStreams']}[a]",
                    "-map",
                    "[a]",
                ]
            )
        ...
        try:
            ...
            for progress in ff.run_command_with_progress(
                utils.createNoWindow(), duration_override=fileInfo["audioDuration"]
            ):
                if self.running == False:
                    ff.quit()
                    break
    ...
    def encodeImage(self, filePath, fileInfo):
        ...
        try:
            if fileInfo["fileFormat"] == "jpeg":
                ...
                )  # jpegoptim is unable to change the output file name so shutil must handle it.
                outputFile = (
                    fileInfo["dirName"]
                    + fileInfo["fileName"]
                    + "_FFmpeg2Discord_Lossless.jpg"
                )
                subprocess.run(  # noqa: PLW1510
                    [self.jpegoptim, "--quiet", "--strip-all", tempFilePath],
                ...
                while True:
                    ...
                    if tempFileSize > self.targetFileSize:
                        if self.imageFormat == "WEBP":
                            ...
                            low = (
                                middle + 2
                            )  # Increased by two because middle always rounds down and for JPG lower means higher file size which leaves the file too big.
                    ...
                    if (  # noqa: PLR1730
                        middle < 0
                    ):  # Prevents the quality value from going below zero which would cause an error in ffmpeg.
                        middle = 0
                    ...
                    if (
                        previousMiddle == middle
                    ):  # The binary search will converge on the optimal quality value. Once this happens the next middle value will be the same as the one before it.
                        break
    ...
    def checkFile(self, outputFile, displayFilePathList, displayFilePath, currentIndex):
        def displayLogs(file, log, gui_log, color):
            ...
            displayFilePathList[currentIndex] = (
                f"<font color={color}>" + displayFilePath + "</font><br>"
            )
        ...
        if outputFile == "error":
            displayLogs(
                displayFilePath,
                "An error has been caught with file",
                "Error with file",
                "red",
            )
            ...
            displayLogs(
                displayFilePath,
                "Error: duration specified is negative",
                "Duration specified is negative",
                "red",
            )
            ...
            displayLogs(
                displayFilePath,
                "Error: bitrate is too low to compress the file",
                "Bitrate is too low",
                "red",
            )
            ...
            displayLogs(
                outputFile, "Error: file does not exist", "Unable to locate file", "red"
            )

        elif (
            os.path.exists(outputFile) == True
            and os.path.getsize(outputFile) * 8 > self.targetFileSize
        ):
            displayLogs(
                outputFile,
                "Error: compression failed to reduce file size below the maximum allowed limit",
                "Outputted file is too large",
                "red",
            )
    ...
    def run(self):
        ...
        for filePath in self.filePathList:
            if filePath:
                ...
                displayFilePathList[currentIndex] = (
                    "<font color='orange'>" + displayFilePath + "</font><br>"
                )
                videoProgress += 1
                ...
                try:
                    if (
                        ...
                        and fileInfo["videoStreams"] != 0
                    ):
                        outputFile = self.encodeVideo(
                            filePath,
                            fileInfo,
                            null,
                            progressPercent,
                    ...
                    elif fileInfo["fileType"] == "audio" or (
                        fileInfo["fileType"] == "video"
                        and fileInfo["videoStreams"] == 0
                    ):
                        outputFile = self.encodeAudio(
                            filePath, fileInfo, audioOnly=True
                        )
                ...
                if self.running == True:
                    self.checkFile(
                        outputFile, displayFilePathList, displayFilePath, currentIndex
                    )
